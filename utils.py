...
def read_csv(filename):
    ...
    labels = [0 if a[1]=='SCD' else 1 for a in your_list[1:]]
...
def data_split(repe_time):
    with open('../../brain2020/lookupcsv/ABCD-SCD-AD.csv', 'r') as f:
        reader = csv.reader(f)
        your_list = list(reader)
    labels, train_valid, test = your_list[0:1], your_list[1:176], your_list[176:]
    for i in range(repe_time):
        ...
        with open(folder + 'train.csv', 'w', newline='') as myfile:
            ...
            wr.writerows(labels + train_valid[:132])
